...
{
    /// <summary>
    /// Stage 1 API Freeze: This file defines stable interfaces and result contracts used by IMKDuckov.* facades and external consumers.
    /// Do not change signatures/semantics in minor patches. See docs/API_FREEZE.md.
    /// </summary>
    ...
    public interface IItemAdapter
    {
        ...
        /// <summary>
        /// 设置变量键的值。
        /// constant 标志指示是否常量；若为 true，应写入常量集合。
        /// </summary>
        ...
        /// <summary>
        /// 设置常量键（可选创建）。
        /// 当 createIfNotExist=true 且键不存在时，需要创建并写入。
        /// </summary>
        ...
        /// <summary>
        /// 重新应用修饰器集合（应在修饰项、统计值等变更后调用以刷新缓存/派生状态）。
        /// </summary>
        ...
        /// <summary>
        /// 设置标签集合（覆盖或合并由实现决定；写服务会提供更精细的 TryWriteTags）。
        /// </summary>
    ...
    { 
        ...
        /// <summary>槽位图标/Sprite（可为空）。</summary>
        public object SlotIcon; 
        /// <summary>需求标签键集合（可为空）。</summary>
        public string[] RequireTagKeys; 
        /// <summary>排除标签键集合（可为空）。</summary>
        public string[] ExcludeTagKeys; 
        /// <summary>是否禁止同 TypeID 物品（集合内 forbidItemsWithSameID 语义）。</summary>
        public bool ForbidSameID; 
        /// <summary>当前内容物品的 TypeID（未占用时 null）。</summary>
        public int? ContentTypeId; 
        /// <summary>当前内容物品的显示名称（未占用时 null）。</summary>
    ...
    /// <summary>
    /// 统一返回结果（不含值）。
    /// 约定：Ok=true 表示操作完整成功（无部分成功），失败时不应该产生持久副作用（除非另有文档说明）。
    /// </summary>
    ...
    /// <summary>
    /// 携带值的统一返回结果。
    /// 约定：Ok=true 表示数据成功返回且完整有效；失败时 Value 为 default。
    /// </summary>
    ...
    public interface IItemMover
    {
        ...
        // Send
        /// <summary>发送到玩家（可选不合并与送到仓库）。</summary>
