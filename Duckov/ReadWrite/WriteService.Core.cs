...
{
    /// <summary>
    /// 写服务（核心字段：Name/RawName/TypeId/Quality/DisplayQuality/Value）。
    /// 契约：若任一字段写入失败，必须将所有字段回滚至写入前状态，返回失败，不抛出给调用方。
    /// </summary>
    internal sealed partial class WriteService : IWriteService
    {
        /// <summary>写核心字段；失败会回滚到写前的快照。</summary>
        public RichResult TryWriteCoreFields(object item, CoreFieldChanges changes)
        {
            try
            {
                ...
                {
                    // Full rollback to pre-write snapshot
                ...
                // Dirty: handled by caller via IMKDuckov.MarkDirty (see docs/DirtyWorkflow.md).
