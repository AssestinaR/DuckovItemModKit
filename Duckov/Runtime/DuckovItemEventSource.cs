...
namespace ItemModKit.Adapters.Duckov
{
    ...
    public sealed class DuckovItemEventSource : IItemEventSource
    {
        ...
        private readonly System.Collections.Generic.Dictionary<int, object> _invRefCache = new System.Collections.Generic.Dictionary<int, object>();
            ...
            try
            {
                ...
                while (_enumerationCursor < _enumBuffer.Count && processed < System.Math.Min(ChunkSize, BudgetPerTick))
                {
                    ...
                    if (!_knownIds.Contains(id))
                    {
                        ...
                        try {
                            var h = ItemModKit.Adapters.Duckov.Locator.DuckovHandleFactory.CreateItemHandle(it);
                            IMKDuckov.RegisterHandle(h);
                            var invObj = it.GetType().GetProperty("InInventory", System.Reflection.BindingFlags.Public|System.Reflection.BindingFlags.NonPublic|System.Reflection.BindingFlags.Instance)?.GetValue(it, null);
                            _invRefCache[id] = invObj;
                        } catch { }
                    }
                    else
                    {
                        int hv = Hash(it);
                        var invCur = it.GetType().GetProperty("InInventory", System.Reflection.BindingFlags.Public|System.Reflection.BindingFlags.NonPublic|System.Reflection.BindingFlags.Instance)?.GetValue(it, null);
                        bool moved = false;
                        if (_invRefCache.TryGetValue(id, out var prevInv))
                        {
                            if (!object.ReferenceEquals(prevInv, invCur)) { moved = true; _invRefCache[id] = invCur; }
                        }
                        else { _invRefCache[id] = invCur; }
                        bool changed = !_hash.TryGetValue(id, out var old) || old != hv;
                        if (changed || moved)
                        {
                            _hash[id] = hv;
                            OnItemChanged?.Invoke(it);
                            IMKDuckov.MarkDirty(it, DirtyKind.All);
                            if (moved)
                            {
                                try { var h = IMKDuckov.TryGetHandle(it); (IMKDuckov.QueryV2 as ItemModKit.Adapters.Duckov.Locator.DuckovItemQueryV2)?.UpdateInventoryEquipped(h); } catch { }
                            }
                            else IMKDuckov.IncrementalRefresh(it);
                        }
                    }
                }
                if (_enumerationCursor >= _enumBuffer.Count)
                {
                    ...
                    foreach (var id in toRemoveIds)
                    {
                        _knownIds.Remove(id);
                        _hash.Remove(id);
                        _invRefCache.Remove(id);
                        if (_idObjCache.TryGetValue(id, out var obj)) { _idObjCache.Remove(id); try { var h = ItemModKit.Adapters.Duckov.Locator.DuckovHandleFactory.CreateItemHandle(obj); IMKDuckov.LogicalIds.Unbind(h); IMKDuckov.UnregisterHandle(h); } catch { } OnItemRemoved?.Invoke(obj); }
                    }
